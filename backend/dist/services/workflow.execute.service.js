...
export class ExecuteSingupWorkFlow {
    async SignupWorkflow(email, idempotent_key, to) {
        ...
        try {
            ...
            // 3. Check idempotency
            const previous_status = await workflow_status.findOne({
                email,
                idempotent_key
            });
            if (previous_status) {
                logger.info(`Workflow already processed for idempotent key ${idempotent_key}`);
            ...
            logger.info(`Workflow execution status set to RUNNING`);
            // 5. Execute workflow
            ...
            if (new_status) {
                ...
                await new_status.save();
                logger.error(`Workflow execution status changed to FAILED for ${email}`);
            }
            throw er;
        }
    }
